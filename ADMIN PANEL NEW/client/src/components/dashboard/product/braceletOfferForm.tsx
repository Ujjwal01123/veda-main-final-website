...
export function BraceletOfferForm() {
  ...
  useEffect(() => {
    const fetchBracelets = async () => {
      try {
        ...
        if (!res.ok) throw new Error("Failed to fetch bracelet");

        const data = await res.json();
        console.log(data.data.bracelet);
        setBraceletList(
          Array.isArray(data.data.bracelets) ? data.data.bracelets : []
        );
      } catch (error: any) {
        toast({
          title: "Error",
  ...
  // Fetch existing bracelet offers
  useEffect(() => {
    const fetchOffers = async () => {
      try {
        const res = await fetch(`${apiUrl}/api/bracelet/offer/all`);
        const data = await res.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingOfferId
        ? `${apiUrl}/api/bracelet/offer/update/${editingOfferId}`
        : `${apiUrl}/api/bracelet/offer/create`;
      ...
      const re = await fetch(`${apiUrl}/api/bracelet/offer/all`);
  ...
  const handleEdit = (offer: any) => {
    setFormData({
      ...
      bracelet: offer.bracelet.map((r: any) => r._id),
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`${apiUrl}/api/bracelet/offer/delete/${id}`, {
        method: "DELETE",
      });
  ...
  const handleToggleActive = async (id: string) => {
    try {
      await fetch(`${apiUrl}/api/bracelet/offer/toggle/${id}`, {
        method: "PATCH",
      });
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        ...
                        variant="outline"
                      >
                        {formData.bracelet.length
                          ? `${formData.bracelet.length} selected`
                          : "Select Bracelet"}
                        <span>▼</span>
                      </Button>
                    </PopoverTrigger>
                    ...
                    <PopoverContent className="w-[300px] max-h-[300px] overflow-y-auto">
                      {braceletList.map((r) => {
                        const selected = formData.bracelet.includes(r._id);
                          ...
                          <div
                            ...
                            onClick={() => {
                              setFormData((prev) => ({
                                ...prev,
                                bracelet: selected
                                  ? prev.bracelet.filter((id) => id !== r._id)
                                  : [...prev.bracelet, r._id],
                              }));
                            }}
                          >
