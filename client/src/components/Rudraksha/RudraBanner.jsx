...
export default function RudraBanner() {
    ...
    useEffect(() => {
        axios
            .get(`${apiUrl}/banners/rudraksha`)
            .then((res) => {
                if (res.data?.success && res.data.data?.images) {
                    setBanners(res.data.data.images)
                }
            })
            .catch((err) => console.error("Error fetching Rudraksha banners:", err))
    }, [apiUrl])
    ...
    if (banners.length === 0) {
        return (
            <section className="relative flex w-full items-center justify-center overflow-hidden bg-gray-100">
                <img src="/images/Artboard-3.jpg" className="w-full object-contain object-center" alt="Fallback Rudraksha Banner" />
            </section>
        )
    }
            ...
            <div className="relative w-full">
                <AnimatePresence mode="wait">
                    <motion.img
                        ...
                        alt={banners[current].altText || "Rudraksha Banner"}
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
...
//             .get(`${apiUrl}/banners/rudraksha`)
//             .then((res) => {
//                 console.log("Fetched banners:", res.data.data?.images)
//                 if (res.data?.success && res.data.data?.images) {
//                     setBanners(res.data.data.images)
//                 }
//             })
...
//                 <img src="/images/Artboard-3.jpg" className="h-full w-full object-cover object-center" alt="Fallback Banner" />
//             </section>
//         )
//     }
