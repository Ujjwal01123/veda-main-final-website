            ...
            <div className="min-h-screen bg-gradient-to-br from-yellow-50 via-orange-50 to-yellow-100 text-gray-800">
                ...
                <section className="mx-auto grid max-w-7xl gap-8 p-6 md:p-8 lg:grid-cols-2">
                    <div className="space-y-4">
                        <div id="mainDisplay" className="soft-border group relative aspect-[6/5] h-[750px] w-full overflow-hidden rounded-2xl">
                            {mainDisplay.type === "image" ? (
                                <img src={mainDisplay.src} alt="Product" className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.03]" data-aos="zoom-in" />
                            ) : (
                ...
                <section className="relative mx-auto mt-12 max-w-7xl px-6">
                    <h2 className="gold-text mb-4 text-center text-3xl text-xl font-bold font-extrabold" data-aos="fade-right">
                        You May Also Like
                    </h2>
                    ...
                    <div ref={carouselRef} id="carousel" className="scrollbar-hide flex space-x-6 overflow-x-auto scroll-smooth pb-4" data-aos="fade-up">
                        {relatedPujas.map((puja, index) => (
                            <div key={index} className="min-w-[250px] rounded-xl border p-3 text-center shadow-md">
                                <img src={puja.img} className="mx-auto h-64 w-64 rounded-lg object-cover transition-transform duration-500 hover:scale-110" alt={puja.name} />
                                <p className="mt-2 font-bold">{puja.name}</p>
                                <p className="font-semibold text-red-600">{puja.price}</p>
                                <button className="hover:shadow-mdc mt-2 transform rounded-lg bg-yellow-500 px-4 py-1 text-white transition duration-300 hover:scale-105 hover:bg-yellow-700">
                                    Book Now
                                </button>
                            </div>
                        ))}
